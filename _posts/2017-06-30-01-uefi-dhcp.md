...
`EFI_DHCPv4_PROTOCOL`もしくは`EFI_DHCPv6_PROTOCOL`を使うと、UEFIでも簡単にDHCPを使ってIPアドレスを取得することができます。
...
ネットワーク系のプロトコルは他とはすこし呼び出し方が異なり、最初に`EFI_SERVICE_BINDING_PROTOCOL`を開いて、それを使って`CreateChild`でHandleにプロトコルを追加し、その上で目的のプロトコルを`OpenProtocol`するというステップが必要です。

また、`EFI_EVENT`を使うことによって、非同期にDHCPの処理を行うこともできます。このへんはまた今度。
